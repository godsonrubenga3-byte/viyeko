...
export default function ProviderDashboard({ requests, onAccept, onComplete, onSendBid, userId }: ProviderDashboardProps) {
  ...
  useEffect(() => {
    ...
    const onNewBreakdown = (message: any) => {
    ...
    channel.subscribe('new-breakdown', onNewBreakdown);
    return () => {
      channel.unsubscribe('new-breakdown', onNewBreakdown);
    };
  }, [profile?.is_online]);
    ...
    <div className="space-y-8 pb-12">
      <div className="glass-card p-6 border-l-4 border-l-slate-yellow flex justify-between items-center">
        ...
        <button 
          ...
          className={cn("px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.2em] transition-all", profile?.is_online ? "bg-emerald-500 text-white shadow-lg shadow-emerald-500/20" : "bg-subtle text-slate-500 border border-subtle")}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {stats.map((stat, i) => (
          <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="glass-card p-6 flex flex-col gap-2 border-subtle relative overflow-hidden group">
            ...
            <div className="flex justify-between items-start relative z-10">
              <div className="p-2 bg-subtle rounded-full text-slate-400"><stat.icon size={20} /></div>
              <span className={cn("text-[10px] font-black px-2 py-0.5 rounded-full bg-subtle", stat.color)}>{stat.trend}</span>
            </div>
            <div className="mt-2">
      ...
      <AnimatePresence mode="wait">
        {biddingRequestId ? (
          <BidInput key="bid-input" requestId={biddingRequestId} serviceTitle={uniqueAvailable.find(r => r.id === biddingRequestId)?.serviceId || 'Service'} onSend={handleSendBid} onCancel={() => setBiddingRequestId(null)} />
        ) : (
            ...
            <div className="space-y-4">
              ...
              {uniqueAvailable.length === 0 ? (
                <div className="py-16 text-center space-y-4 glass-card border-dashed border-subtle bg-transparent">
                  <div className="bg-subtle w-16 h-16 rounded-full flex items-center justify-center mx-auto text-slate-600"><Activity size={32} className="animate-pulse" /></div>
                  <div className="space-y-1">
                    <p className="text-slate-100 font-bold uppercase tracking-widest text-xs">Waiting for Broadcasts...</p>
                    <p className="text-slate-500 text-[10px]">New requests appear here.</p>
                  </div>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {uniqueAvailable.map((req) => {
                    ...
                    return (
                      <motion.div key={req.id} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-card p-6 space-y-4 border-l-4 border-l-slate-yellow group hover:border-l-emerald-500 transition-all">
                        <div className="flex justify-between items-start">
                          <div className="flex items-center gap-4">
                            <div className={cn("p-3 rounded-full text-white shadow-lg", service?.color)}>{service && <service.icon size={24} />}</div>
                            <div>
                        ...
                        <div className="space-y-3 pt-4 border-t border-subtle">
                          <div className="flex items-center gap-3 text-sm text-slate-300"><MapPin size={16} className="text-viyeko-red" /><span className="font-medium truncate">{req.location.address}</span></div>
                          <div className="flex items-center gap-3 text-sm text-slate-300"><Car size={16} className="text-slate-500" /><span className="font-medium">{req.vehicleInfo}</span></div>
                        </div>
                        <div className="pt-4">
                          {myBid ? (
                            ...
                            <button onClick={() => setBiddingRequestId(req.id)} className="w-full h-12 bg-slate-yellow text-charcoal rounded-full text-xs font-black uppercase tracking-widest hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 shadow-lg shadow-slate-yellow/10">Quote Your Price <ArrowUpRight size={18} /></button>
                          )}
                        </div>
                      </motion.div>
                    );
                    ...
                    return (
                      <div key={req.id} className="glass-card p-6 border-l-4 border-l-emerald-500 space-y-4">
                        <div className="flex justify-between items-start">
                          <div className="flex items-center gap-4">
                            <div className={cn("p-3 rounded-full text-white shadow-lg", service?.color)}>{service && <service.icon size={24} />}</div>
                            <div><h4 className="font-black text-slate-100 text-lg leading-tight uppercase tracking-tight">{service?.title}</h4><p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{req.status}</p></div>
                          </div>
                          <div className="text-right"><p className="text-[10px] font-black text-slate-yellow italic">TZS {req.totalCost?.toLocaleString()}</p></div>
                        </div>
                        <button 
