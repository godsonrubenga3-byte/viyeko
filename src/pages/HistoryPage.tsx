...
export default function HistoryPage({ requests }: HistoryPageProps) {
  ...
  const historyRequests = requests.filter(r => r.status === 'completed' || r.status === 'canceled');

  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-black text-slate-100 italic tracking-tight uppercase">Rescue History</h2>
        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{historyRequests.length} Total</span>
      </div>
            ...
            return (
              <motion.div 
                ...
                className={cn(
                  ...
                  req.status === 'completed' ? "border-l-emerald-500" : "border-l-rose-500"
                )}
              >
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex flex-col items-end gap-1">
                    <div className={cn(
                      ...
                      req.status === 'completed' ? "bg-emerald-900/30 text-emerald-400 border border-emerald-500/20" : "bg-rose-900/30 text-rose-400 border border-rose-500/20"
                    )}>
                      {req.status}
                    </div>
                    {req.lastUpdatedBy && (
                      <span className="text-[8px] font-black text-slate-600 uppercase tracking-tighter">
                        By {req.lastUpdatedBy}
                      </span>
                    )}
                  </div>
                </div>
